...
const errors = {
  ...
  'stellar-auth.errors.challenge.invalid-response':
...
export default class IndexController extends Controller {
  ...
  async authenticate(btn) {
    ...
    const allowHttp =
      authEndpoint && authEndpoint.toLowerCase().startsWith('http://');
    const opts = {
      authAccount,
      authEndpoint,
      allowHttp,
    ...
    try {
      this.lastRequest.token = await auth.loginWithSecret(this.secretKey);
    } catch (e) {
      ...
      if (this.lastRequest) {
        this.lastRequest.challengeEncoded = encodeURIComponent(
          this.lastRequest.challenge
        );
        this.lastRequest.signedChallengeEncoded = encodeURIComponent(
          this.lastRequest.signedChallenge
        );
